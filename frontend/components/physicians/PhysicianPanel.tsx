...
// ── Bulk lead capture modal ───────────────────────────────────────────────────
// Shown when user clicks "Load More" — captures a lead for ALL currently shown physicians
...
function BulkLeadCaptureModal({ physicians, site, onClose }: BulkLeadModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Submit a lead record for each currently visible physician
      const promises = physicians.map((p) =>
        fetch("/api/leads", {
          method: "POST",
  ...
  return (
    ...
    >
      <div style={{
        ...
        boxShadow: "0 32px 80px rgba(0,0,0,0.20)", overflow: "hidden",
        fontFamily: "'Sora', sans-serif",
      }}>
        {/* Header */}
        <div style={{
          ...
          {/* Physician preview chips */}
          <div style={{
            display: "flex", flexWrap: "wrap", gap: 5, marginTop: 12,
            maxHeight: 72, overflow: "hidden",
          }}>
            {physicians.slice(0, 8).map((p) => (
              <span key={p.npi} style={{
        ...
        <div style={{
          ...
          {site.facility && <span>Site: <strong style={{ color: "#0f172a" }}>{site.facility}</strong></span>}
        </div>
            ...
            <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: 14 }}>
              ...
              <div>
                ...
                <textarea value={form.message} onChange={set("message")}
                  placeholder="Notes about this outreach…" rows={3}
            ...
            {[
              ...
              <div key={i} style={{
                ...
                <div style={{
                  ...
                  color: "#0f172a", fontFamily: "'IBM Plex Mono', monospace",
                }}>{s.value}</div>
        ...
        {searched && !loading && physicians.length > 0 && (
          <div style={{
            ...
            {(["map", "list"] as Tab[]).map((t) => (
              ...
              >
                {t === "map"
                  ? (<><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z"/><circle cx="12" cy="10" r="3"/></svg> Map</>)
        ...
        <div style={{ flex: 1, overflow: "auto" }}>
          ...
          {loading && (
            <div style={{
              ...
              gap: 12, paddingTop: 56, color: "#94a3b8",
            }}>
          ...
          {!loading && error && (
            <div style={{
              margin: "16px", padding: "14px 16px", borderRadius: 10,
          ...
          {!loading && !error && !searched && (
            <div style={{
              display: "flex", flexDirection: "column",
              alignItems: "center", justifyContent: "center",
              gap: 10, paddingTop: 56, textAlign: "center",
              color: "#94a3b8",
            }}>
          ...
          {!loading && !error && physicians.length > 0 && tab === "map" && (
            <div style={{ padding: 16 }}>
              ...
              {/* Distance note */}
              <p style={{
                fontSize: 11, color: "#94a3b8", marginTop: 10, textAlign: "center",
                fontStyle: "italic",
              }}>
          ...
          {/* ── Load More + Bulk Lead Capture ── */}
          {!loading && !error && physicians.length > 0 && total > physicians.length && (
            <button
              className="pp-load-more-btn"
              onClick={() => setShowBulkLead(true)}
            >
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5">
                <path d="M12 5v14M5 12l7 7 7-7"/>
              </svg>
              Load more ({total - physicians.length} remaining) — Capture leads first
            </button>
          )}
          ...
          {!loading && !error && physicians.length > 0 && total <= physicians.length && (
            <p style={{
              ...
              color: "#cbd5e1", padding: "12px 16px",
              fontStyle: "italic",
            }}>
