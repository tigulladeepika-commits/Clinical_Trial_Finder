...
// v7: expanded map = modal overlay with blurred backdrop
...
function suggestedMarkerHtml(color: string, size: number, selected: boolean): string {
  const glow = selected ? `filter:drop-shadow(0 0 8px ${color});` : "filter:drop-shadow(0 2px 6px rgba(0,0,0,0.22));";
  return `
    <div style="${glow}cursor:pointer;transition:transform 0.15s;">
      <svg xmlns="http://www.w3.org/2000/svg" width="${size}" height="${size}" viewBox="0 0 30 30">
        <circle cx="15" cy="15" r="14" fill="${color}" stroke="white" stroke-width="${selected ? 3 : 2}"/>
...
export default function PhysicianMap({
  ...
  const initMap = () => {
    ...
    if (!document.getElementById("phys-map-style-v7")) {
      const style = document.createElement("style");
      style.id = "phys-map-style-v7";
      style.textContent = `
        ...
        .pm-modal-panel {
          ...
          animation: pmModalIn 0.26s cubic-bezier(.22,1,.36,1) both;
        }
        .pm-modal-close {
        ...
        .pm-modal-close:hover { background: #fee2e2; color: #dc2626; border-color: #fecaca; }
      `;
    ...
    const map = L.mapquest.map(mapDivRef.current, {
      center: [centerLat, centerLng], layers: initialLayer,
    ...
    const buildPopupHtml = (p: Physician, accentColor: string, bgColor: string, borderColor: string, badge?: string) => `
      <div>
        <div style="padding:13px 16px 11px;border-bottom:1px solid ${borderColor};background:${bgColor};">
          ${badge ? `<div style="margin-bottom:4px;"><span style="background:${accentColor};color:#fff;font-size:9px;font-weight:700;padding:2px 7px;border-radius:20px;">${badge}</span></div>` : ""}
      ...
      marker.bindPopup(buildPopupHtml(p, "#2563eb", "#f0f9ff", "#dbeafe") + `
        <div style="padding:8px 16px 12px;">
          <button onclick="window.__viewPhysician && window.__viewPhysician('${p.npi}')"
            style="width:100%;padding:7px 0;background:#2563eb;color:white;border:none;border-radius:8px;font-size:12px;font-weight:600;cursor:pointer;font-family:'Sora',sans-serif;">
            View Details →
          </button>
        </div>`, { className: "phys-popup", offset: [0, -10], maxWidth: 280, closeButton: true });
    ...
    const suggestedMarkers = mappableSuggested.map((p) => {
      ...
      marker.bindPopup(buildPopupHtml(p, "#14b8a6", "#f0fdfa", "#99f6e4", "SUGGESTED") + `
        <div style="padding:8px 16px 12px;">
          <button onclick="window.__viewPhysician && window.__viewPhysician('${p.npi}')"
            style="width:100%;padding:7px 0;background:#14b8a6;color:white;border:none;border-radius:8px;font-size:12px;font-weight:600;cursor:pointer;font-family:'Sora',sans-serif;">
            View Details →
          </button>
        </div>`, { className: "phys-popup-suggested", offset: [0, -10], maxWidth: 280, closeButton: true });
      marker.on("mouseover", () => marker.openPopup());
      marker.on("click", () => {
        map.flyTo([p.lat, p.lng], Math.max(map.getZoom(), 14), { animate: true, duration: 0.8 });
        onSelect(p);
    ...
    if (allPoints.length > 1) map.fitBounds(window.L.latLngBounds(allPoints), { padding: [40, 40] });
  };
  ...
  useEffect(() => {
    ...
    if (!document.getElementById("mc-css")) {
      const l = document.createElement("link"); l.id = "mc-css"; l.rel = "stylesheet";
    ...
    if (!document.getElementById("mc-css2")) {
      const l = document.createElement("link"); l.id = "mc-css2"; l.rel = "stylesheet";
    ...
    if (!document.getElementById("mq-js")) {
      ...
      script.onload = () => {
        if (!document.getElementById("mc-js")) {
          ...
          mc.onload = loadAndInit; document.head.appendChild(mc);
        } else { loadAndInit(); }
      };
    ...
    return () => {
      ...
      markersRef.current = []; tileLayerRef.current = null; radiusCircleRef.current = null;
    };
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [
    mapKey, selectedSite.lat, selectedSite.lng, selectedSite.facility,
    physicians.map(p => p.npi).join(","),
    suggestedPhysicians.map(p => p.npi).join(","),
  ]);

  // reactive radius circle update
  useEffect(() => {
  ...
  if (!mapKey) {
    return (
      <div style={{ height: "100%", display: "flex", alignItems: "center", justifyContent: "center", flexDirection: "column", gap: 10, background: "#f8fafc", color: "#94a3b8" }}>
  ...
  // ── Shared overlay controls (legend + zoom + type switcher) ──────────────
  const MapOverlayControls = () => (
    <>
      ...
      <div style={{
        ...
        borderRadius: 10, padding: "9px 13px", boxShadow: "0 2px 8px rgba(0,0,0,0.08)",
      ...
      <div style={{ position: "absolute", top: 10, right: 10, zIndex: 1000, display: "flex", flexDirection: "column", gap: 4, alignItems: "center" }}>
        <div style={{ width: 32, height: 20, background: "white", border: "1px solid #e2e8f0", borderRadius: 6, fontSize: 10, fontWeight: 700, color: "#475569", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "'IBM Plex Mono', monospace" }}>
          {currentZoom}
        </div>
        {[
          ...
          <button key={b.title} title={b.title} onClick={b.fn} disabled={b.disabled}
            style={{
              ...
              fontSize: 16, fontWeight: 700, color: b.disabled ? "#cbd5e1" : "#334155",
      ...
      <div style={{ position: "absolute", bottom: 14, right: 10, zIndex: 1000 }}>
        {showTypeMenu && (
          <div style={{ position: "absolute", bottom: "calc(100% + 6px)", right: 0, background: "white", border: "1px solid #e2e8f0", borderRadius: 10, boxShadow: "0 6px 20px rgba(0,0,0,0.12)", overflow: "hidden", minWidth: 140 }}>
            {MAP_TYPES.map((t) => (
        ...
        <button onClick={() => setShowTypeMenu((v) => !v)} title="Change map type"
          style={{
            ...
            background: "white", border: `1px solid ${showTypeMenu ? "#2563eb" : "#e2e8f0"}`,
            borderRadius: 8, cursor: "pointer", fontSize: 11, fontWeight: 600, color: "#334155",
            boxShadow: "0 2px 6px rgba(0,0,0,0.08)", fontFamily: "'Sora', sans-serif",
          }}>
      ...
      <div style={{ position: "relative", width: "100%", height: "100%", overflow: "hidden" }}>
        ...
        <button
          ...
          style={{
            ...
            cursor: "pointer", boxShadow: "0 2px 8px rgba(0,0,0,0.12)", fontSize: 14, color: "#374151",
          }}
        >⛶</button>
      </div>
      ...
      {isExpanded && (
        ...
        >
          <div className="pm-modal-panel">
            {/* Close button */}
            <button className="pm-modal-close" onClick={() => setIsExpanded(false)} title="Close">✕</button>

            {/* Map canvas inside modal — Leaflet container is moved here */}
            <div
